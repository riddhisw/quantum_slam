...
\begin{algorithm}[H] % Floats not comptatible with revtek4-1. Need "[H] option. 
	...
	\begin{algorithmic}[0] 
		\Procedure{qslamr}{$ \mathrm{Qubit Grid}$}\Comment{Input $N$ nodes}
		\\
		\If{$ t = 0$}
		...
		\State Sample $x_{0}^{(\alpha)} \sim \prob{}{x_{0}}$ \Comment{??}
...
\begin{algorithm}[H] % Floats not comptatible with revtek4-1. Need "[H] option. 
	...
	\begin{algorithmic}[0] 
		...
		\State Generate $\{\beta_\alpha, x_t^{(\alpha, \beta_\alpha)} \} $ by $\rest{j_t}{t} \sim \mathcal{U}(0,L) $ \Comment{\cref{eqn:slam:PF:13}}
		\For{$\beta_\alpha \in P_\beta$}
		\State Generate $\qmsmt{q \alpha, \beta_\alpha}{t}$ \Comment{\cref{eqn:physetting:quantiser:2,eqn:physetting:quantiser:3}}
...
\\
%If resampling is initiated, we sample $P_\alpha$ particles with probability given by $ W_t^{(\alpha, \beta_\alpha |j_t)}$. Once these posterior samples have been generated, we replace all weights to $(P_\alpha P_\beta)^{-1}$.\\
